...
module.exports = class ScanTools{
    ...
    async startScan(scanConfigId) {
        ...
        if(response.status < 200 || response.status > 299) {
            core.error(`Unable to start Rapid7 Application Security scan: ${response.status}, ${response.data}`);
            throw Error("Unable to get Rapid7 Application Security scan.");
        }
        ...
        if(!scanId) {
            core.error(`Scan UUID was not returned in the Rapid7 Application Security response`);  
        }
    ...
    async pollForScanComplete(scanId, startTime = null) {
        ...
        do {
            ...
            if(scan.status < 200 || scan.status > 299 || !scan.data) {
                core.error(`Unable to get Rapid7 Application Security scan: ${scan.status}, ${scan.data}`);
                throw Error(`Unable to get Rapid7 Application Security scan: ${scan.status}, ${scan.data}`);
            }
    ...
    async getScanResultsSummary(scanId, vulnQuery, scanConfigId) {
        ...
        do {
            ...
            if(results.status < 200 || results.status > 299) {
                core.error(`Unable to get Rapid7 Application Security scan results: ${results.status}, ${results.data}`);
                return null;
            }
    ...
    postScanActions(scanId, status, isTimeout) {
        if(ERROR_STATUS.includes(status)) {
            core.info(`The scan is in an error state ${status}. Check Rapid7 Application Security for details.`);
        }
        else if (isTimeout) {
